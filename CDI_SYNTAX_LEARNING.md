...
- File: src/main/webapp/WEB-INF/beans.xml
    ...
    - <scan><exclude> com.arjuna.ats.jta.cdi.NarayanaTransactionManager, NarayanaUserTransaction,
      NarayanaTransactionSynchronizationRegistry </scan>
...
- @Transactional (jakarta.transaction.Transactional): demarcates transactions around methods/classes. Requires JTA and
  interceptor on classpath (Narayana provides it when bundled).
...
- Do not try to bind Narayana objects into Tomcat’s java:comp JNDI — it’s read-only in app context; use static accessors
  with producers (as we do).
